	...
	client.on('message-new', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: '🌜 Falha, tenta de novo macaco 🌛',
					Iv: '🌜 Link inválido 🌛'
				},
				only: {
					group: '🌜so em grupo neh poha se e burro  🌛',
					ownerG: '🌜 Este comando só pode ser usado pelo dono do grupo aquele 🐒 🌛',
					ownerB: '🌜 Este comando só pode ser usado pelo dono do bot 🤏🏻🥺! 🌛',
					admin: '🌜Da adm pro bot né poha ! 🌛 ',
					Badmin: '🌜So pode ser usado quando o bot virar um administrador! 🌛 '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["554298653614@s.whatsapp.net"] // replace this with your number
			...
			switch(command) {
				...
				case 'gtts':
					...
					if (args.length < 2) return client.sendMessage(from, 'Onde está o texto, tio?, text, {quoted: mek})
					dtt = body.slice(9)
					ranm = getRandom('.mp3')
					rano = getRandom('.ogg')
					dtt.length > 600
				...
				case 'loli':
					loli.getSFWLoli(async (err, res) => {
						if (err) return reply('https://imgur.com/gallery/ZTi2LUW')
				...
				case 'url2img':
					...
					if (!tipelist.includes(args[0])) return reply('Tipo de área de trabalho | tablet | Móvel ')
					if (args.length < 2) return reply('Cadê o url, macaco?')
				...
				case 'tstiker':
				case 'tsticker':
					if (args.length < 1) return reply('Cadê o texto,poha?')
					ranp = getRandom('.png')
					rano = getRandom('.webp')
        ...
        case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Parabens Gorila @${mentioned[0].split('@')[0]} Ganhou Adm Nessa Poha🐒!`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Esse Macaco Aqui 🐒 @${mentioned[0].split('@')[0]} Virou Comum Podem Zuar 😂🤏🏻 !`, mentioned, true)
				...
				case 'kick':
					...
					if (mentioned.length > 1) {
						...
						mentions(` Macaco tomou ban🐒fica ativo pra não tomar também: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
				case 'listadmins':
				...
				case 'toimg':
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('🌜 Falha ao converter tenta de novo macaco 🌛')
				...
				case 'welcome':
					...
					if (args.length < 1) return reply('Hmmmm')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('Já ativo gay')
						welkom.push(from)
